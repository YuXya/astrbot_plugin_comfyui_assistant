...
> 0.1.2 为测试版本：自动化与本机 ComfyUI 验证已通过，QQ 实际收图尚待联调。
...
工作流 → 编辑 →「图片大小写入缩放」默认 1。填写 0.5 时，AI 传入 1280×720，插件向绑定节点写入 640×360；开始提示及任务中的原始宽高仍为 1280×720。小数和不能被 8 整除的结果会自动四舍五入到最近的 8 像素倍数，中点向上，至少 8 像素。不额外发送取整提示，实际写入尺寸保存在任务记录中。AI 没有传宽高时，保留工作流模板尺寸，不应用倍率。

原始宽高继续遵守服务设置；缩放后的宽高及总像素也不能超过服务设置上限。倍率只控制尺寸入口，不会改变工作流里的放大器；最终图片大小由整个工作流决定。例如后续放大 4 倍时，可按需要填写 0.25。

服务设置 →「AI 回复编辑（开始提示）」可以填写：

```text
在画了老大，大小：{size}
```

支持 `{size}`（如 1280×720）、`{width}`、`{height}`、`{workflow}`，始终使用 AI 传入的原始尺寸。不传尺寸时，`{size}` 为「默认尺寸」，宽高分别为「默认」。普通大括号用 `{{` 和 `}}`；不支持其他占位符或格式表达式。

开始提示由插件发送，创建任务时保存文本。修改模板和倍率只影响新任务；已有任务重启恢复时沿用原请求和提示，重发只发送原图。默认仍为开始提示和最终结果两次发送。
