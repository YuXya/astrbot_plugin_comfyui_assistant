...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{
   ...
   window.fixture={version:'0.1.1',settings:{server_url:'http://127.0.0.1:8188',min_edge:256,max_edge:2048,multiple:8,max_pixels:2097152,record_days:30,image_days:7,poll_seconds:3,tracking_minutes:30},workflows:['文生图','图生图','多图生图'].map((name,i)=>({id:String(i),name:'Krea2'+name,enabled:true,short:'根据描述与参考图完成绘制。',detailed:'测试说明',graph,bindings})),tasks:[{id:'test-task',workflow_name:'Krea2文生图',generation:'completed',delivery:'failed',start:'sent',error:'平台未确认发送成功',created:Date.now()/1000,width:768,height:1024,scope:'fixture:GroupMessage:100',owner:'fixture',attempts:[],files_expired:false}]};
  ...
  for(const width of [1500,390]){
   ...
   assert.equal(await prefix.inputValue(),configured);assert.equal(await target.inputValue(),second);
  ...
  assert.deepEqual(errors,[]);console.log('Desktop/mobile UI passed: prefix save/clear, stable target, deleted-target validation, upload, delete, edit, settings, resend, no image previews, no overflow.');
