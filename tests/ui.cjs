...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{
   const graph={'51':{class_type:'Simple String',inputs:{string:'test'}},'82':{class_type:'EmptyLatentImage',inputs:{width:1024,height:1024,batch_size:1}},'29':{class_type:'SaveImage',inputs:{images:['82',0]}}};
   const bindings={texts:[{node:'51',input:'string',label:'绘画描述'}],images:[],width:[{node:'82',input:'width'}],height:[{node:'82',input:'height'}],outputs:['29']};
   window.fixture={version:'0.1.0',settings:{server_url:'http://127.0.0.1:8188',min_edge:256,max_edge:2048,multiple:8,max_pixels:2097152,record_days:30,image_days:7,poll_seconds:3,tracking_minutes:30},workflows:['文生图','图生图','多图生图'].map((name,i)=>({id:String(i),name:'Krea2'+name,enabled:true,short:'根据描述与参考图完成绘制。',detailed:'测试说明',graph,bindings})),tasks:[{id:'test-task',workflow_name:'Krea2文生图',generation:'completed',delivery:'failed',start:'sent',error:'平台未确认发送成功',created:Date.now()/1000,width:768,height:1024,scope:'fixture:GroupMessage:100',owner:'fixture',attempts:[],files_expired:false}]};
  ...
  for(const width of [1500,390]){
   ...
   await page.getByRole('button',{name:'编辑',exact:true}).first().click();await page.getByLabel('工作流名称',{exact:true}).fill('Krea2文生图-测试');await page.getByLabel('详细介绍',{exact:true}).fill('新的详细介绍');await page.getByRole('button',{name:'保存工作流',exact:true}).click();await page.locator('dialog').waitFor({state:'hidden'});
   ...
   await page.locator('[data-tab=settings]').click();await page.getByLabel('最大边长',{exact:true}).fill('1920');await page.getByRole('button',{name:'保存设置',exact:true}).click();
  ...
  assert.deepEqual(errors,[]);console.log('Desktop/mobile UI passed: upload, delete, edit, settings, resend, no image previews, no overflow.');
